...
#максимально допустимый вариант соопщение в телеграмм
...
#tz4
@bot.callback_query_handler(func=lambda call: call.data in ["tz4_40", "tz4_custom"])
def callback_tz4(call):
    ...
    if call.data == "tz4_40":
        msg = StudentsAttendanceReport(user_files[user_id], 40)
        send_long(chat_id, msg)
        return

    if call.data == "tz4_custom":
        tz4_state[user_id] = "input_percent"
        bot.send_message(chat_id, "Введите процент посещаемости (0-100):")

#tz5
@bot.callback_query_handler(func=lambda call: call.data in ["tz5_70", "tz5_custom"])
def callback_tz5(call):
    user_id = call.from_user.id
    chat_id = call.message.chat.id
    bot.answer_callback_query(call.id)
    ...
    if call.data == "tz5_70":
        tz5_data[user_id] = {"percent": 70}
        tz5_state[user_id] = "select_period"
        select_period(chat_id)
        return

    if call.data == "tz5_custom":
        tz5_state[user_id] = "input_percent"
        bot.send_message(chat_id, "Введите процент (0-100):")
        return

@bot.callback_query_handler(func=lambda call: call.data in ["tz5_month", "tz5_week"])
def callback_tz5_period(call):
    user_id = call.from_user.id
    chat_id = call.message.chat.id
    bot.answer_callback_query(call.id)

    if user_id not in tz5_data:
        bot.send_message(chat_id, "Ошибка: нет процента.")
        return

    period = "month" if call.data == "tz5_month" else "week"
    percent = tz5_data[user_id]["percent"]
    file_path = user_files[user_id]

    tz5_last_period[user_id] = period

    msg = HomeworkCheckedReport(file_path, period, percent)

    kb = types.InlineKeyboardMarkup()
    if period == "month":
        kb.add(types.InlineKeyboardButton("Посмотреть за неделю", callback_data="tz5_switch_week"))
    else:
        kb.add(types.InlineKeyboardButton("Посмотреть за месяц", callback_data="tz5_switch_month"))

    bot.send_message(chat_id, msg, reply_markup=kb)

@bot.callback_query_handler(func=lambda call: call.data in ["tz5_switch_week", "tz5_switch_month"])
def callback_tz5_switch(call):
    user_id = call.from_user.id
    chat_id = call.message.chat.id
    bot.answer_callback_query(call.id)

    if user_id not in tz5_data or user_id not in tz5_last_period:
...
def select_period(chat_id):
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("За месяц", callback_data="tz5_month"))
    kb.add(types.InlineKeyboardButton("За неделю", callback_data="tz5_week"))
...
#Процент для tz5
    if user_id in tz5_state and tz5_state[user_id] == "input_percent":
        try:
            value = float(text)
            if not (0 <= value <= 100):
        ...
        tz5_data[user_id] = {"percent": value}
        tz5_state[user_id] = "select_period"
        select_period(chat_id)
        return

# Процент для tz5
    if user_id in tz4_state and tz4_state[user_id] == "input_percent":
        try:
            value = float(text)
            if not (0 <= value <= 100):
                bot.send_message(chat_id, "Введите число от 0 до 100.")
            ...
            bot.send_message(chat_id, "Введите число, например: 45")
            return

        msg = StudentsAttendanceReport(user_files[user_id], value)
        send_long(chat_id, msg)
        del tz4_state[user_id]
        return
    ...
    if text == "Расписание групп":
        ...
        kb = types.InlineKeyboardMarkup()
        kb.add(types.InlineKeyboardButton("Меньше 40%", callback_data="tz4_40"))
        kb.add(types.InlineKeyboardButton("Ввести свой %", callback_data="tz4_custom"))
        bot.send_message(chat_id, "Выберите фильтр:", reply_markup=kb)

    elif text == "Проверенные ДЗ":
        kb = types.InlineKeyboardMarkup()
        kb.add(types.InlineKeyboardButton("Меньше 70%", callback_data="tz5_70"))
        kb.add(types.InlineKeyboardButton("Ввести свой %", callback_data="tz5_custom"))
        bot.send_message(chat_id, "Выберите фильтр:", reply_markup=kb)
